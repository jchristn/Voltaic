...
- A JSON Schema `pattern` that .NET's ECMAScript mode cannot compile (for example one with `\p{...}` Unicode property escapes) runs on .NET's engine; `$`, `.`, `\d`, `\w`, and `\s` keep their ECMA-262 meaning there too.
