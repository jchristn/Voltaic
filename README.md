...
- Unicode property escapes in a JSON Schema `pattern` accept .NET's own names (such as `\p{IsGreek}` blocks) as well as the ECMA-262 general category names (`\p{L}`, `\p{Letter}`, `\p{gc=Lu}`).
- An error response to a message whose ID cannot be read carries `"id": null`, as JSON-RPC 2.0 requires.
...
| `ResponseKeepAliveMs` defaults to 0 (15000 from v2.1.10) | Servers relying on keep-alives for disconnect detection | Set it above 0; see "Status codes and disconnects on HTTP" |
