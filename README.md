...
- **Tools can be removed.** `UnregisterTool(name)` removes a tool and returns `true` if it existed. Registering and unregistering tools does not notify clients by itself; call `NotifyToolsChanged()` (HTTP) or `NotifyToolsChangedAsync()` (stdio, TCP, WebSocket) afterwards to send `notifications/tools/list_changed`.
...
Voltaic implements the MCP revisions `2024-11-05`, `2025-03-26`, `2025-06-18`, `2025-11-25`, and `2026-07-28`, and follows their MUST and SHOULD requirements on every transport (stdio, TCP, WebSocket, and Streamable HTTP). This section lists the optional features Voltaic does not implement and the few places where it is deliberately more lenient than the specification.

**Optional features not implemented.**

| Area | Voltaic behavior | Specification |
|---|---|---|
| HTTP+SSE transport (2024-11-05) | Not implemented, and `McpHttpClient` does not fall back to it; `/rpc` and `/events` are different, Voltaic-specific endpoints | Deprecated since 2025-03-26. `2024-11-05` itself is supported over Streamable HTTP, stdio, TCP, and WebSocket. |
| Server-to-client requests (sampling, elicitation, roots) on handshake-era sessions | Not sent by Voltaic servers | Optional. On `2026-07-28`, tool handlers can return `McpInputRequiredResult` (see [Multi Round-Trip Requests](#asking-the-user-for-input-multi-round-trip-requests)); a handshake-era caller gets an `isError` result instead. Voltaic clients answer such requests from other servers through `RegisterRequestHandler`. |
...
| OAuth authorization | Resource-server plumbing only: `ProtectedResourceMetadata`, `BearerChallenge`, `InsufficientScope`, `McpInsufficientScopeException`, and `AuthenticationHandler` | The authorization server, token validation, and audience checks are application responsibilities. See [OAuth and protected resource metadata](#oauth-and-protected-resource-metadata). |
...
- `McpHttpClient` treats a `2026-07-28` result without `resultType` as complete, for servers that predate the field. Unknown `resultType` values are rejected.
...
**Status codes after a response stream has started.** The HTTP status is sent with the first byte of the response. A POST response becomes an SSE stream (status 200) when the handler sends a progress or log notification, or, for `2026-07-28` requests, when the first keep-alive is written after `ResponseKeepAliveMs`. An error that occurs after that point, such as a `-32021` from a tool's input request or an `McpInsufficientScopeException`, arrives as the final event of the stream instead of as a 400 or 403 response. The keep-alive is the only way `HttpListener` can detect that a client closed the stream, which `2026-07-28` requires servers to treat as cancellation. Tools can avoid the late `-32021` by checking `McpToolCallContext.ClientSupports` before requesting input. Setting `ResponseKeepAliveMs = 0` keeps responses unstreamed until a notification is sent, at the cost of disconnect detection.

**Behavior that follows the specification**, for reference:

- Lifecycle: `initialize` must come first (other requests get `-32600`, except `ping`), must carry `protocolVersion`, `capabilities`, and `clientInfo` (`-32602`), and is accepted once per session. A version the server does not know is answered with `MaximumHandshakeProtocolVersion`. Every Voltaic client initializes on connect (`AutoInitialize`).
- JSON-RPC: `id` must be a string or number (`null` is `-32600`), `jsonrpc` must be `"2.0"`, and MCP `params` must be an object. Batches are rejected on `2025-06-18` and later (an empty batch is `-32600`, and `initialize` may not be batched); every client accepts batches from the server. A handler that returns null answers `{}`, and params members of the wrong JSON type are `-32602`.
- Concurrency and cancellation: requests on one connection run concurrently, so `ping` is answered while a tool runs. `notifications/cancelled` cancels the handler's token and suppresses its response and any further progress or log messages, including when it arrives before its request; clients send it when a call times out or is cancelled, and honor it for requests the server sent them. On `2026-07-28` HTTP, closing the response stream cancels the request (detected with keep-alives, `ResponseKeepAliveMs`), and `McpHttpClient` re-issues a request whose stream broke.
- Notifications: `list_changed` goes only to initialized sessions; `resources/updated` only to sessions subscribed to that URI; `notifications/message` only at or above the session's `logging/setLevel` level (or the `2026-07-28` `_meta` log level); progress only for a request in flight that carries the token, with increasing values, on the request's own response stream (server-level `NotifyProgressAsync` never guesses between clients that use the same token). Notifications and results are downgraded to what the negotiated revision defines (for example audio content becomes text for `2024-11-05`).
- Features: tool names are validated (1-128 characters of letters, digits, `_`, `-`, `.`); input and output schemas are objects; a tool with an `outputSchema` must return `structuredContent`, and `structuredContent` that is not an object is removed before `2026-07-28` (the text block keeps it); resource-not-found is `-32002` on handshake-era revisions and `-32602` on `2026-07-28`; resource templates follow RFC 6570 (reserved and fragment values, explode, dotted variable names); completions return at most 100 values with the full `total`; cursors are opaque and stable across changes.
- HTTP: invalid `Origin` gets 403; sessions come only from a successful `initialize`; a missing session gets 400 and an unknown or terminated one 404; `DELETE` ends a session and only its owner may send it; `McpHttpClient` starts a new session after a 404 and sends `DELETE` when it disconnects. POST response streams on a session carry event IDs (with a priming event from `2025-11-25`) and can be resumed with `GET` and `Last-Event-ID`. A 401 always carries `WWW-Authenticate`, every Bearer challenge names `resource_metadata` when `ProtectedResourceMetadata` is set, and missing scopes get 403 `insufficient_scope`.
- `2026-07-28`: `_meta` must carry the protocol version and `clientCapabilities` (`-32602`); results carry `resultType` and `serverInfo` in `_meta`; `ping`, `logging/setLevel`, and `resources/subscribe` are not served; input requests need the matching client capability, including `elicitation.url` for URL mode and `sampling.tools` for tool use (`-32021`); `server/discover` requires the `_meta` protocol version; `MCP-Protocol-Version`, `Mcp-Method`, `Mcp-Name`, and `Mcp-Param-{Name}` headers are validated (400 `-32020`); an unsupported version gets 400 `-32022` with the versions this server accepts, and `McpHttpClient` retries with one of them. An `x-mcp-header` argument must be mirrored whatever its JSON type (an object or array cannot be, and is rejected).
