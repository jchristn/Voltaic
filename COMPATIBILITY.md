...
- **Voltaic version:** 2.1.9
- **Last reviewed:** 2026-09-26. The v2.1.8 source was reviewed line by line against every MUST, MUST NOT, SHOULD, and SHOULD NOT requirement of each revision (four independent reviews: lifecycle and stream transports, Streamable HTTP and authorization, server features and data model, and the stateless revision); v2.1.9 resolves each finding, and each resolution is covered by a test that fails against v2.1.8. A review of v2.1.9 is pending.
...
| Cancellation (races, reused IDs, nothing after cancellation, reasons logged) | ✅ | ✅ | ✅ | ✅ | ✅ | Servers and clients; includes rejected and malformed requests and timed-out pings |
| Progress | ✅ | ✅ | ✅ | ✅ | ✅ | |
...
| Authorization, resource server (401/403 challenges, `resource_metadata`, protected resource metadata at the resource's own URL) | — | ✅ ⁶ | ✅ | ✅ | ✅ | |
...
| A `pattern` that .NET's ECMAScript mode cannot compile (such as `\p{...}`) runs on .NET's engine, with `$`, `.`, `\d`, `\w`, and `\s` still translated to their ECMA-262 meaning | 2025-11-25, 2026-07-28 | Such patterns would otherwise be unusable |
